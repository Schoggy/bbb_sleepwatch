...
int start_watch_thread(WTHR *thread, char sensnr, unsigned int delay_ms, void *(*func) (void *)){
  int response;
  ...
  // create the thread
  response = pthread_create(&(thread->t_id), &thread_attr, func, (void*) thread);
...
int start_other_thread(OTHR *thread, unsigned int delay_ms, void *(*func) (void *)){
  int response;
