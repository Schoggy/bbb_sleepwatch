  ...
  if(dht_res != DHT_SUCCESS){
    switch(dht_res){
      case DHT_ERROR_TIMEOUT : {
        logn("WARNING DHT did not return in give timeframe: DHT_ERROR_TIMEOUT!", dht_res);
      } break;
      case DHT_ERROR_CHECKSUM : {
        logn("INFO DHT checksum failed to validate: DHT_ERROR_CHECKSUM!", dht_res);
      } break;
      case DHT_ERROR_ARGUMENT : {
        logn("ERROR DHT read function failed! Wrong input to read function: DHT_ERROR_ARGUMENT!", dht_res);
      } break;
      case DHT_ERROR_GPIO : {
        logn("ERROR DHT is not connected correctly or wrong pin numbers are used: DHT_ERROR_GPIO!", dht_res);
      } break;
      default:{
...
void add_to_buf(char sensnr, int* val){
  *((bufarr + sensnr)->w_ptr) = *val;
  (bufarr + sensnr)->w_ptr = (bufarr + sensnr)->s_ptr + (((bufarr + sensnr)->w_ptr - (bufarr + sensnr)->s_ptr + 1) % (bufarr + sensnr)->bufsize);
}


int grab_value(char sensnr){
  int out = *((bufarr + sensnr)->r_ptr);
  (bufarr + sensnr)->r_ptr = (bufarr + sensnr)->s_ptr + (((bufarr + sensnr)->r_ptr - (bufarr + sensnr)->s_ptr + 1) % (bufarr + sensnr)->bufsize);
  return out;
}
...
int flush_buffer_to_db(void){
  ...
  for(cntr_o = 0; cntr_o < 5; cntr_o++){
    akk = 0;
    for(cntr_i = 0; (bufarr + cntr_o)->r_ptr == (bufarr + cntr_o)->w_ptr; cntr_i++){
