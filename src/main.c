...
int main(int argc, char* argv[]) {
  ...
  if(output_mins == 0){ output_mins = DEFAULT_OUTPUT_MINS;}
  
  // sets output update delay default
  if(out_delay == 0){ out_delay = DEFAULT_OUT_DELAY;}
...
void print_usage(void){
  printf("Usage: sleepwatch [OPTIONS]\n\n"
          "Options:\n"
          "  -d=foo/bar  Path to and name of database file. Default is current directory.\n"
          "  -f=foo/bar  Path to and name of output file. Default is current directory.\n" 
          "  -l=foo/bar  Path to and name of log file. Default is current directory.\n"
          ...
          "  -w=x        Wait x seconds between refreshing hte output file. Default is 30 seconds.\n"
          "\n");
}
